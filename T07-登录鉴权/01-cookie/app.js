...
const randomKeys = {};
const users = [
  { name: 'root', password: cipher('root'), role: 'root' },
  { name: 'admin', password: cipher('admin'), role: 'admin' },
];
...
async function authHandler(ctx, next) {
  ...
  if (witeList.includes(ctx.path)) {
    ...
    const password = ctx.cookies.get('password', { signed: true });
    // 进行相关判断
...
router.post('/login', (ctx, next) => {
  ...
  // 登录成功便跳转首页并设置cookie
  if (users.find(item => item.name === name && decipher(item.password) === password)) {
    ctx.cookies.set('name', name, { signed: true });
    ctx.cookies.set('password', cipher(password), { signed: true });
    ctx.body = '登录成功, 跳转到首页';
  } else {
